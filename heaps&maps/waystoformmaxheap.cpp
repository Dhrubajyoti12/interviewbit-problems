...
bint solve(const int n){
    if(n < 3)   return 1;
    pii childz = subtrees(n);
    int l = childz.left;
    int r = childz.rite;
printf("l:%d, r:%d\n",l, r);
    return (((C(n-1, l)*solve(l))%MOD)*solve(r))%MOD;
}

int main(void){
    init_table();
    // int b, p; while(cin >> b >> p)  cout<<povar(b, p)<<endl;
    // int n, k; while(cin >> n >> k)  cout<<C(n, k)<<endl;
    // int nana; while(cin >> nana)    cout<<'\t'<<height(nana)<<endl;
    // int size; while(cin >> size)    bastir(size);
    int size; while(cin >> size)    cout<<solve(size)<<endl;
    return 0;
}